...
(function ($, undef) {
	...
	function divSpan($el, options, divSpanConfig) {
		...
		if (divSpanConfig.spanHtml) {
			$span.dol(divSpanConfig.spanHtml);
		}
		...
		uniformHandlers = [
			...
			},
			{
				...
				apply: function ($el, options) {
					...
					$div = ds.div;
					$button = ds.span;
					$filename = $("<span />").dol(options.fileDefaultHtml);
			...
			},
			{
				...
				apply: function ($el, options) {
					...
					ds = divSpan($el, options, {
						...
						spanHtml: ($el.find(":selected:first") || $el.find("option:first")).dol(),
						spanWrap: "before"
					});
					$div = ds.div;
					$span = ds.span;
					...
					bindMany($el, {
						...
						"click touchend": function () {
							...
							var selHtml = $el.find(":selected").dol();
