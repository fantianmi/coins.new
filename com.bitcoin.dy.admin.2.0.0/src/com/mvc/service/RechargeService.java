...
public class RechargeService {
	...
	public List<Object> getByStateForOrders(int i){
		List<Object> list = entityDao.createQuery("select bro from Btc_rechargeCNY_order bro where bro.bro_state='" + i + "' order by bro.bro_recharge_time desc");
		if (list.size() != 0) {
			return list;
		}else{
			return null;
	...
	public int countRow(int i){
		List<Object> list = entityDao.createQuery("select count(bro.bro_id) from Btc_rechargeCNY_order bro where bro.bro_state='" + i + "'");
      if (list.size() != 0) {
           if(list.get(0)!=null){
	...
	public List<Object> getByStateForOrders(int i,int start,int count){
		List<Object> list = entityDao.createQuery("select bro from Btc_rechargeCNY_order bro where bro.bro_state='" + i + "' order by bro.bro_recharge_time desc",start,count);
		if (list.size() != 0) {
			return list;
		}else{
			return null;
	...
	public List<Object> getByStateForWithdrawCNYOrders(int i){
		List<Object> list = entityDao.createQuery("select bwo from Btc_withdrawCNY_order bwo where bwo.btc_bwo_state='" + i + "' order by bwo.btc_bwo_time desc");
		if (list.size() != 0) {
			return list;
		}else{
			return null;
	...
	public int countRowWithdraw(int i) {
        List<Object> list = entityDao.createQuery("select count(bwo.btc_bwo_id) from Btc_withdrawCNY_order bwo where bwo.btc_bwo_state='" + i + "'");
        if (list.size() != 0) {
             if(list.get(0)!=null){
                  return Integer.parseInt(list.get(0).toString());
             }
	...
	public List<Object> getByStateForWithdrawCNYOrders(int i,int start,int count){
		List<Object> list = entityDao.createQuery("select bwo from Btc_withdrawCNY_order bwo where bwo.btc_bwo_state='" + i + "' order by bwo.btc_bwo_time desc",start,count);
		if (list.size() != 0) {
			return list;
		}else{
			return null;
	...
	public Btc_withdrawCNY_order getByBwoIdForWithdrawCNYOrders(int i){
		Btc_withdrawCNY_order bwo = new Btc_withdrawCNY_order();
		List<Object> list = entityDao.createQuery("select bwo from Btc_withdrawCNY_order bwo where bwo.btc_bwo_id='" + i + "'");
		if (list.size() != 0) {
			bwo = (Btc_withdrawCNY_order)list.get(0);
			return bwo;
		}else{
			return null;
		}	
	...
	public BigDecimal totalWithdraw(int uid){
		String sql="select sum(bwo.btc_bwo_amount) from Btc_withdrawCNY_order bwo where bwo.btc_bwo_state=0";
		if(uid!=0)sql=sql+" and bwo.uid="+uid+"";
	...
	public void deleteWithdrawCNY_Order(Btc_withdrawCNY_order bwo) {
		entityDao.delete(bwo);
	}
	
	public void deleteRechargeCNY_Order(Btc_rechargeCNY_order bro) {
		entityDao.delete(bro);
	}
}
