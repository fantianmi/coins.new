...
public class RechargeStockService {
	...
	public List<Object> getAllByUid(int uid){
		List<Object> list = entityDao.createQuery("select brso from Btc_rechargeStock_order brso where brso.uid='" + uid + "' order by brso.date desc");
		if (list.size() != 0) {
			return list;
		}else{
			return null;
	...
	public Btc_rechargeStock_order getById(int id){
		List<Object> list = entityDao.createQuery("select brso from Btc_rechargeStock_order brso where brso.id='" + id + "'");
		if (list.size() != 0) {
			return (Btc_rechargeStock_order)list.get(0);
		}else{
			return null;
		}	
	}
	...
	public List<Object> getAll(){
		List<Object> list = entityDao.createQuery("select brso from Btc_rechargeStock_order brso order by brso.date desc");
		if (list.size() != 0) {
			return list;
		}else{
			return null;
	...
	public void deleteOrder(Btc_rechargeStock_order brso) {
		entityDao.delete(brso);
	}
}
