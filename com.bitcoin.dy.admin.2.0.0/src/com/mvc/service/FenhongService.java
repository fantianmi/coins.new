...
//		List<Object> buylist = entityDao.createQuery("select sum(bdl.btc_deal_total) from Btc_deal_list bdl where bdl.buid=" + uid + " and DATE(bdl.btc_deal_time)=DATE_SUB(CURDATE(),INTERVAL 1 DAY)");
	...
	//###########################################################################################
	public boolean isInput(int season) {
		List<Object> list = entityDao.createQuery("select bfl.fenhong_id from Btc_fenhonglog bfl where bfl.fenhong_season='"+season+"'");
		if (list.size() != 0) {
			return true;
		}
		return false;
	...
	public boolean isTongjiFenhong(int season) {
		List<Object> list = entityDao.createQuery("select bfl.fenhong_id from Btc_fenhonglog bfl where bfl.fenhong_season='"+season+"' and bfl.fenhong_status='未统计'");
		if (list.size() != 0) {
			return false;
		}
		return true;
	...
	public Btc_fenhonglog getLatestBtc_fenhonglog() {
		List<Object> list = entityDao.createQuery("select bfl from Btc_fenhonglog bfl where bfl.fenhong_status='未统计'");
		if (list.size() != 0) {
			return (Btc_fenhonglog)list.get(0);
		}
		return null;
	}
	
	public void saveBtc_fenhonglog(Btc_fenhonglog bfl) {
		entityDao.save(bfl);
	}
