...
public class ZhifubaoService {
	...
	public List<Object> getZhifubaoByStatus(String status,int start,int count) {
		List<Object> list = entityDao.createQuery("SELECT zhi from Btc_zhifubao zhi where zhi.status='"+status+"'",start,count);
		if (list.size() != 0) {
			return list;
		}else{
			return null;
	...
	public int countRow(String status) {
        List<Object> list = entityDao.createQuery("SELECT count(zhi.id) from Btc_zhifubao zhi where zhi.status='"+status+"'");
        if (list.size() != 0) {
             if(list.get(0)!=null){
