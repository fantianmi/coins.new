...
public class StockOrderService {
	...
	public List<Object> getBtc_inout_orderByUid(int uid){
		List<Object> list = entityDao.createQuery("select order from Btc_inout_order order where order.uid='"+uid+"' order by order.btc_inout_time");
		if (list.size() != 0) {
			return list;
		}else{
			return null;
	...
	public List<Object> getBtc_inout_orderByid(String status, String type){
		List<Object> list = entityDao.createQuery("select order from Btc_inout_order order where order.btc_inout_status='"+status+"' and order.btc_inout_type='"+type+"'");
		if (list.size() != 0) {
			return list;
		}
		return list;
