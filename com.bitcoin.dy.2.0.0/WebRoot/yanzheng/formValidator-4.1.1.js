...
$.fn.formValidator = function(cs) 
{
	...
	return this.each(function(e)
	{
		...
		if(fixTip.length==1 && onMouseOutFixTextHtml!="" && onMouseOnFixTextHtml!="" && showFixText != "")
		{
			jqobj.hover(
				function () {
					fixTip.dol(onMouseOnFixTextHtml.replace(/\$data\$/g, showFixText));
				},
				function () {
					fixTip.dol(onMouseOutFixTextHtml.replace(/\$data\$/g, showFixText));
				}
			);
			fixTip.css("padding","0px 0px 0px 0px").css("margin","0px 0px 0px 0px").dol(onMouseOutFixTextHtml.replace(/\$data\$/g, setting.onShowFixText)); 
		}
		//获取输入框内的提示内容
		    ...
		    jqobj.bind("keyup",function(){
		        if(this.validatorPasswordIndex > 0)
		        {
					try{
						...
						$("#"+setting.pwdTipID).dol(passwordStrengthStatusHtml[level]);
					}catch(e)
					{
