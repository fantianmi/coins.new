...
function createXMLHttpRequest() {
 ...
 function getGamesdata() {
      ...
      var url = 'autoload.do?luckwheel&n='+ Math.random();
  ...
  <!-- luckwheelsrc -->
		...
		$("#startbtn").rotate({
			bind:{
				click:function(){
				    ...
				    $.ajax({ 
				        type: 'POST', 
				        url: 'luckwheel.do?luckwheel&zhushu='+izhushu+'&stockid='+sstockid, 
				        dataType: 'json', 
				        cache: false, 
				        error: function(){ 
				            alert('您的余额不足'); 
	...
	<!-- luckwheelsrc -->
      ...
      <div style="margin-left: 40px; padding-top: 5px;border-bottom: 5px solid #0171C7;" id="usercenter">
        ...
        游戏说明：可以以平台里面任意币种为单位，根据下注数量计算中奖数量，如下注数量为A，一等奖为<%=lres.getString("一等奖") %>A，二等奖为<%=lres.getString("二等奖") %>A，三等奖为<%=lres.getString("三等奖") %>A，四等奖为<%=lres.getString("四等奖") %>A，五等奖为<%=lres.getString("五等奖") %>A，六等奖为<%=lres.getString("六等奖") %>A，七等奖为<%=lres.getString("七等奖") %>A；系统按照概率从【一等奖、二等奖、三等奖、四等奖、五等奖、六等奖、七等奖】中随机七选一，中奖概率为50%；下注最小数量为<%=lres.getString("min.zhu") %>，最大数量为<%=lres.getString("max.zhu") %>；中奖记录可以‘我的交易’中进行查看；<span class="sell-color">【免责声明】：此游戏仅供茶余饭后小玩怡情，请大家切勿沉迷其中，该游戏对个人带来的不良影响与损失平台概不负责！</span>
        </div>
        <!-- alert row -->
        ...
        <p><button onclick="javascript:window.location.href='index.do?ordermanage'" class="btn small blue">查看中奖纪录</button></p>
        </div>
        <!-- table row -->
        </div>
      </section>
