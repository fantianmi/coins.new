...
public class HoldingUtil {
	...
	public boolean subStock(int uid,int stockid,BigDecimal amount){
		...
		if(hs.getBtc_holding(uid, stockid)==null){
			...
			Btc_holding hold = hs.getBtc_holding(uid, stockid);
			if(hold.getBtc_stock_amount().subtract(amount).compareTo(new BigDecimal(0))<0) return false;
	...
	public boolean frozenstock(int stockid,BigDecimal amount,int uid){
		if(hs.getBtc_holding(uid, stockid)==null)return false;
		Btc_holding hold = hs.getBtc_holding(uid, stockid);
		if(hold.getBtc_stock_amount().compareTo(amount)<0) return false;
		hold.setBtc_stock_amount(hold.getBtc_stock_amount().subtract(amount));
		
		if(hold.getFrozen_amount()==null)hold.setFrozen_amount(amount);
		else hold.setFrozen_amount(hold.getFrozen_amount().add(amount));
		hs.updateBtc_holding(hold);
		
		return true;
	}
	public boolean defrozenstock(int stockid,BigDecimal amount,int uid){
		if(hs.getBtc_holding(uid, stockid)==null)return false;
		Btc_holding hold = hs.getBtc_holding(uid, stockid);
		if(hold.getFrozen_amount()==null) return false;
		if(hold.getFrozen_amount().compareTo(amount)<0) return false;
		hold.setBtc_stock_amount(hold.getBtc_stock_amount().add(amount));
		hold.setFrozen_amount(hold.getFrozen_amount().subtract(amount));
		hs.updateBtc_holding(hold);
		return true;
	}
