...
public class FormController {
	...
	public void trade(@RequestParam("stock_id") String stock_id,
		...
		Btc_stock stock=stockService.getBtc_stockById(Integer.parseInt(stock_id));
		if(stock.getIstrade()==0){
			msg = "对不起，"+stock.getBtc_stock_name()+"停盘中";
			out.println("<response>");
			out.println("<href>" + href + "</href>");
			out.println("<msg>" + msg + "</msg>");
			out.println("</response>");
			...
			if (btc_order_type.equals("bid")) {
				...
				if (btc_holding != null) {
					...
					if (holding_stock.compareTo(btc_order_amount) == -1) {
						...
						out.println("</response>");
						out.close();
						matchAlgorithmUtil2.matchAlgorithm(btc_stock_id, "sell");
	...
	public void stocktrade(@RequestParam("stock_id") String stock_id,
		...
		if(exstock.getIstrade()==0){
			msg = "对不起，"+exstock.getBtc_stock_name()+"停盘中";
			out.println("<response>");
			out.println("<href>" + href + "</href>");
			out.println("<msg>" + msg + "</msg>");
			out.println("</response>");
