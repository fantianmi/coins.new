...
@Controller
@RequestMapping("/checkGivenCoinAndRemove.do")
public class checkGivenCoinAndRemove {
	ResourceBundle res = ResourceBundle.getBundle("farm"); 
	@Autowired
	private HoldingService holds;
	@Autowired
	private HoldingUtil holdutil;
	protected final transient Log log = LogFactory.getLog(checkGivenCoinAndRemove.class);
	...
	public void load(
		...
		hold=holds.getBtc_holding(tuid, Integer.parseInt(res.getString("toolstockid")));
		if(hold==null){
			obj.put("flag", 1);
		...
		holdutil.subStock(tuid, Integer.parseInt(res.getString("toolstockid")), amount);
