	...
	public void loadRereshByStock(
		...
		if (btc_sellBTC_order_list != null) {
			...
			while (it.hasNext()) {
				...
				globalsellorder+="<li onclick='set_bprice("+btc_sellBTC_order.getBtc_order_price()+","+btc_sellBTC_order.getBtc_order_amount()+","+format.trans((btc_sellBTC_order.getBtc_order_price().multiply(btc_sellBTC_order.getBtc_order_amount())))+");'>";
				globalsellorder+="<span>"+ format.trans(btc_sellBTC_order.getBtc_order_price())+ "</span>";
				globalsellorder+="<span>"+ format.trans(btc_sellBTC_order.getBtc_order_amount())+ "</span>";
				globalsellorder+="<span>"+ format.trans(btc_sellBTC_order.getBtc_order_price().multiply(btc_sellBTC_order.getBtc_order_amount()))+ "</span></li>";
				i++;
				if(i==10)break;
			}
		} else {
			globalsellorder +="<li><span>暂无数据</span><span>暂无数据</span><span>暂无数据</span></li>";
		}
		// buyorderlist
		...
		if (orderService.getBuyingOrders(stockId,exstock) != null) {
			...
			while (it.hasNext()) {
				...
				globalbidorder+="<li onclick='set_sprice("+btc_rechargeBTC_order.getBtc_order_price()+","+btc_rechargeBTC_order.getBtc_order_amount()+","+btc_rechargeBTC_order.getBtc_order_price().multiply(btc_rechargeBTC_order.getBtc_order_amount()).setScale(2, BigDecimal.ROUND_HALF_UP)+");'>";
				globalbidorder+="<span>"+ format.trans(btc_rechargeBTC_order.getBtc_order_price())+ "</span>";
				globalbidorder+="<span>"+ format.trans(btc_rechargeBTC_order.getBtc_order_amount())+ "</span>";
				globalbidorder+="<span>"+ format.trans(btc_rechargeBTC_order.getBtc_order_price().multiply(btc_rechargeBTC_order.getBtc_order_amount()))+ "</span></li>";
				i++;
				if(i==10)break;
			}
		} else {
		...
		if (btc_deal_list_LIST != null) {
			for (int i = 0; i < btc_deal_list_LIST.size(); i++) {
				...
				dealorderlist+="<li><span>"+ btc_deal_list.getBtc_deal_time()+ "</span>";
				dealorderlist+="<span>" + type+ "</span>";
				dealorderlist+="<span>"+ format.trans(btc_deal_list.getBtc_deal_Rate())+ "</span>";
				dealorderlist+="<span>"+ format.trans(btc_deal_list.getBtc_deal_quantity())+ "</span>";
				dealorderlist+="<span>"+ format.trans(btc_deal_list.getBtc_deal_total()) + "</span></li>";
				count ++;
			}
		} else {
			dealorderlist += "<li><span>暂无记录</span><span>暂无记录</span><span>暂无记录</span><span>暂无记录</span><span>暂无记录</span></li>";
		}
		//#####################user buy list#####################################
		String userbuyorder = "";
		...
		//#############对数组进行排序#####################//
      	...
      	while(it.hasNext()){
      		...
      		userbuyorder+="<li><span class='buy-color'>"+format.trans(userbido.getBtc_order_price())+"</span>";
      		userbuyorder+="<span>"+format.trans(userbido.getBtc_order_amount())+"</span>";
      		userbuyorder+="<span>"+format.trans(userbido.getBtc_order_price().multiply(userbido.getBtc_order_amount()))+"</span>";
      		userbuyorder+="<span><button class='button small black' onclick='ConfirmDelbid("+userbido.getBtc_order_price()+");' >撤单</button></span></li>";
      	}
		}else{
				userbuyorder +="<li><span>暂无数据</span><span>暂无数据</span><span>暂无数据</span><span>暂无数据</span></li>";
		}
		...
		List<Object> userbidList = orderService.getSellingOrdersByUid(stockId, user.getUid(),exstock);
      	...
      	while(it.hasNext()){
      		BigDecimal key = (BigDecimal)it.next();
      		userbido = pricemap.get(key);
      		usersellorder+="<li><span class='sell-color'>"+format.trans(userbido.getBtc_order_price())+"</span>";
      		usersellorder+="<span>"+format.trans(userbido.getBtc_order_amount())+"</span>";
      		usersellorder+="<span>"+format.trans(userbido.getBtc_order_price().multiply(userbido.getBtc_order_amount()))+"</span>";
      		usersellorder+="<span><button class='button small black' onclick='ConfirmDelsell("+userbido.getBtc_order_price()+");' >撤单</button></span></li>";
      	}
			}else{
				usersellorder+="<li><span>暂无数据</span><span>暂无数据</span><span>暂无数据</span><span>暂无数据</span></li>";
			}
		}else{//如果用户未登录
			usersellorder+="<li><span>暂无数据</span><span>暂无数据</span><span>暂无数据</span><span>暂无数据</span></li>";
