...
public class MailController {
	...
	public void sendforactive(
		...
		body = body + username + "</div><div>如确认无误，请点击下方按钮激活该邮箱，完成修改注册邮箱的操作。</div><div><a href='"+url+"'>"+url +"</a></div><div>欢迎您注册"+hostres.getString("host.title")+"交易中心，谢谢。</div><div>&nbsp;</div><div id='spnEditorSign'><span style='color: rgb(0, 0, 0); font-family: arial; font-size: 14px; line-height: 23px;'>"+hostres.getString("host.small.title")+"</span>&nbsp;&nbsp;网址：<a href='"+hostres.getString("host.wangzhi")+"'>"+hostres.getString("host.wangzhi")+"<div style='color: rgb(0, 0, 0); font-family: arial; font-size: 14px; line-height: 23px;'>";
		Btc_mail_content content = new Btc_mail_content();
		content.setBtc_mail_content_body(body);
		content.setBtc_mail_content_id(1);
		content.setBtc_mail_content_subject(""+hostres.getString("host.title")+"账号激活邮件");
		content.setBtc_mail_content_use("active");
		mailUtil.sendMail(content, user.getUemail());
		msg = "已重新发送激活邮件，请查收并尽快激活";
		href = "index.do?userdetail";
		out.println("<response>");
		out.println("<href>" + href + "</href>");
		out.println("<msg>" + msg + "</msg>");
		out.println("</response>");
