...
public class LuckWheelService {
	...
	public List<Object> getMyGameLog(int uid,int start,int count) {
		List<Object> list = entityDao.createQuery("SELECT lw from Games_luckwheel lw where lw.uid="+uid+" order by lw.id desc",start,count);
		if (list.size() != 0) {
			return list;
		}
		return null;	
	...
	public List<Object> getLuckWheel(String type,int start,int count) {
		List<Object> list = entityDao.createQuery("SELECT lw from Games_luckwheel lw where lw.type='"+type+"' order by lw.id desc",start,count);
		if (list.size() != 0) {
			return list;
		}else{
			return null;
